# ARGUS — технические подробности

[English](DETAILS.md) · **Русский**

Как всё устроено ниже рендереров и грабли, каждая из которых стоила отладки: OpenComputers,
GregTech/GTNH, OCGlasses, Applied Energistics, сети и доставка файлов. Здесь то, что дорого
выяснять заново.
...
Распространяется под **GPL-3.0**; происхождение и заимствования — в [NOTICE.ru.md](NOTICE.ru.md).
Взят графический слой, всё остальное вырезано, слой данных и панель энергии переписаны заново.
...
`nil` (именно из-за этого раньше не определялся Battery Buffer). Резолвить через `component.get`.
...
**Строки сенсора парсить по ЛЕЙБЛАМ, не по индексам.** Наивный разбор берёт `[2]`, `[5]`,
`[23]` — аддон вставляет строку, и вместо ошибки получаются молча неверные числа.
...
- Сигналы: `glasses_on(user, w, h)`, `glasses_off(user)`, `hud_click(user, x, y, button)`,
  `hud_keyboard(user, char, key)`, `block_interact`, `overlay_opened/closed`. **По факту
  сигнал несёт ведущий адрес очков**: живой дамп показал `hud_keyboard(<UUID>, user, char,
  key)` — первый аргумент срезается в `hud:handleSignal` по UUID-паттерну.
- **`char` в `hud_keyboard` — мусор, клавиши вязать на СКАН-КОД `key`.** OCGlasses шлёт
  Java-`char`, но в дампе `[`→27, `]`→29 (управляющие коды, не ASCII `91/93`). Скан-код же
  верен и стабилен: `[`=26, `]`=27, `-`=12, `=`=13, `←`=203, `→`=205, `1`..`9`=2..10, `f`=33,
  `c`=46. Ранняя версия сравнивала `char` с числом — **все символьные хоткеи молча не
  работали**, а стрелки жили только потому, что одни были на `key`. Фикстуры тестов обязаны
  слать реальную форму (скан-код + мусорный char), иначе баг проходит мимо.
...
## Грабли Applied Energistics (крафты)

**Крафтовое API есть ТОЛЬКО в форке GTNH.** Апстримный OC про запущенное задание не даёт
ничего, кроме `isDone()` на объекте из своего же `request()` — открытый
[issue #3786](https://github.com/MightyPirates/OpenComputers/issues/3786). Всё стоит на
`NetworkControl.scala` из `1.11.20-GTNH`: `getCpus()` → `{name, storage, coprocessors,
busy, cpu}`, а значение `cpu` умеет `activeItems`, `pendingItems`, `storedItems`,
`finalOutput`, `isBusy`, `isActive`, `cancel`. Сверять по тегу, не по ocdoc.cil.li — там
описан апстрим, и этих методов в документации нет вообще.

**`finalOutput()` требует блок Crafting Monitor в кластере CPU.** Драйвер ищет
`TileCraftingMonitorTile` в `getCpu.getTiles` и берёт `getJobProgress()`; без него —
`(null, "No crafting monitor")`. Программного обхода нет: больше драйверу итог задания
взять неоткуда. Цепочка при этом читается нормально — неизвестен только итог, поэтому UI
пишет, какой блок поставить, а не `?`.

**Порядка в цепочке не существует.** Задание — дерево параллельных подзадач;
`getListOfItem(PENDING)` отдаёт **множество**. «Следующий» может быть только эвристикой
(берётся самый большой стек). Выдавать его за очередь — врать пользователю; на странице
про это написано прямо.

**Признака остановки нет.** `isStalled()` не существует, состояние выводится из того, что
показания не меняются — отсюда история в `core/craft.lua`. Ключевое: **чистый таймаут не
годится**, один рецепт GT идёт минутами (порог 120 сек). Сильный признак другой — занят,
`pendingItems` непусты, а `activeItems` пусты: работа есть, но в машины ничего не ушло.
Здорового чтения такой формы почти не бывает, порог 15 сек.

**Списки предметов бывают с ДЫРКАМИ.** `convert()` в драйвере возвращает `null` для стека,
который не смог описать, и в Lua это дырка в массиве. `ipairs` остановится на первой и
молча недосчитает работу — обходить `pairs`. Фикстуры обязаны воспроизводить дырку.

**Историю простоя чистить при освобождении CPU.** `readCpu` для незанятого CPU выходит
рано (экономия вызовов) — если не сбросить запись там, следующее задание с той же
сигнатурой (тот же рецепт заказали дважды) унаследует чужой таймер и сразу станет STALLED.
Тест на это есть; баг был реальный.

**Опрос дорогой**: `getCpus()` + до 4 вызовов на **занятый** CPU. Свой интервал (2 сек),
незанятые не читаются.
...
**Сервер общий — базы надо разделять.** `modem.broadcast` получает любой модем в радиусе,
открывший тот же порт, а порт по умолчанию у всех один. Без ключа сервер соседа опрашивал бы
наши клиенты, а наш сервер собирал бы чужие базы. Каждое сообщение несёт `net.PROTOCOL` и
**ключ сети**; `net:accepts()` — единственная точка проверки. Ключ по умолчанию выводится из
`computer.address()` (уникален, стабилен). Это **SSID, а не пароль**: трафик открытый,
шифрования в OC нет. Настоящая изоляция — только Linked Card (точка-точка).
...
tests/run.lua         419 тестов, десктопный Lua
...
**В игре пока не тестировалось** — фикстуры сенсора реконструированы по Java-исходникам.
Если цифры расходятся — снять `tools/sensordump.lua` и открыть issue.
...
4. Тесты + `luac -p`
5. Коммит, `git tag -a vX.Y.Z`, push ветки и тега
...
- [OpenComputers @1.11.20-GTNH: `NetworkControl.scala`](https://github.com/GTNewHorizons/OpenComputers/blob/1.11.20-GTNH/src/main/scala/li/cil/oc/integration/appeng/NetworkControl.scala) — весь крафтовый API AE2: `getCpus`, `Cpu`, `convert`
...
- Крафты AE2 не проверены в игре: фикстуры реконструированы по `NetworkControl.scala`.
  Если страница пуста или цифры странные — снять `tools/medump.lua` и открыть issue.
...
- Отмена зависшего задания из очков: `cancel()` в API есть, сознательно не сделано —
  решено ограничиться мониторингом, чтобы случайный клик не убил многочасовой заказ.
