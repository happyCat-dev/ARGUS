...
Как соединять сети — [раздел в README](README.ru.md#несколько-баз-сервер-и-клиенты).
...
Графический слой взят у проекта-предшественника (происхождение — в [NOTICE.ru.md](NOTICE.ru.md));
всё остальное (мультиблоки, инфузии, флюиды, сеть серверов) вырезано, слой данных и панель
написаны заново.
...
Ключевые решения, найденные при разборе исходников GT5U `5.09.51.482`:

- **Парсинг по лейблам, а не по индексам.** Наивный разбор берёт `[2]`, `[5]`, `[23]` — аддон
...
- Типичные баги наивной реализации не допущены: иначе Battery Buffer не определялся бы
  (`component.list()` ключуется полным UUID, а не сокращённым адресом); состояния-таблицы
  ломались бы после сериализации; в `getInteger` сохранён знак.
- **Требования к железу ниже**: вместо рендера через буферы видеопамяти (GPU T3) ARGUS
