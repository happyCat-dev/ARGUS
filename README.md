...
```
 ...
 ▁▂▃▅▆▇█▇▆▅▃▂▁▂▃▅▆▇█▇▆▅▃▂▁▂▃▅▆▇█▇▆▅▃▂▁                                       9.2E EU
 ──────────────────────────────────────────────────────────────────────────────────────
 [ Dashboard ] [ Buffers ] [ Glasses ] [ Network ] [ Graph: 10 min ] [ set ] [ Save ]
```
...
```shell
cd /home
wget -f https://cdn.jsdelivr.net/gh/sblndn20/ARGUS@v2.3.0/setup.lua && setup
```
...
```shell
wget -f https://raw.githubusercontent.com/sblndn20/ARGUS/v2.3.0/setup.lua && setup
```
...
setup --branch=v2.3.0                  # конкретный тег
...
**NET / IN / OUT / LOSS** — что происходит **сейчас**, в EU/t. `NET` = `IN − OUT − LOSS`,
то есть насколько буфер реально наполняется или пустеет.
...
Прочерк в `received`/`sent` означает, что источник не сообщает проход энергии за окно —
такие цифры даёт LSC. Для беспроводной сети и хранилищ IC2 их взять неоткуда, и вместо
выдуманного нуля стоит `--`.
