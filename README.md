...
```shell
cd /home
wget -f https://cdn.jsdelivr.net/gh/sblndn20/ARGUS@v2.1.0/setup.lua && setup
```
...
```shell
wget -f https://raw.githubusercontent.com/sblndn20/ARGUS/v2.1.0/setup.lua && setup
```
...
setup --branch=v2.1.0                  # конкретный тег
...
2. **Port** — любой, но **одинаковый везде**. Модем молча выбрасывает сообщения на не
   открытый порт, поэтому расхождение выглядит как «клиентов нет».
3. **This node** — имя базы: с ним её буферы появятся у сервера (`Шахта · LSC`).
4. **Poll** и **timeout** (только сервер): как часто опрашивать и через сколько секунд тишины
   считать базу отключённой.
5. **Save** на обеих. Смена роли применяется сразу, перезапуск не нужен.
...
- **Порт разный.** Самая частая причина.
